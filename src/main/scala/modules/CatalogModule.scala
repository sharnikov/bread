...
import services.{CatalogService, CatalogServiceImpl, NewItem, NewOrder}
import settings.JsonParsers._
import settings.schedulers.MainContext

class CatalogModule(ordersDao: OrdersDAO, sessions: ConcurrentHashMap[String, Date])
  ...
  def routes(catalogService: CatalogService): Route = Route.seal(
    get {
      path("all_goods") {
